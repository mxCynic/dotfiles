...
function Timeline:get_visibility()
	if options.autohide and not cursor.hidden then
		return 1
	end
...
function Timeline:update_dimensions()
	...
	local bottom_gap = round(options.bottom_gap * state.scale)
	self.ax = window_border_size
	self.ay = display.height - window_border_size - bottom_gap - self.size - self.top_border
	self.bx = display.width - window_border_size
	self.by = display.height - window_border_size - bottom_gap
	...
	local available_space = display.height - window_border_size * 2 - bottom_gap - Elements:v('top_bar', 'size', 0)
...
function Timeline:render()
	...
	local bar_radius = math.max(1, math.floor(math.min((bby - bay) / 2, (bbx - bax) / 2)))
	local progress_radius = math.max(1, math.floor(math.min((fby - fay) / 2, math.abs(fbx - fax) / 2)))
	...
	if is_line then
		...
		ass:rect(bax, bay, bbx, bby, {
			color = bg,
			opacity = config.opacity.timeline,
			radius = bar_radius,
		})
	end
	...
	-- Media title above the timeline
	if text_opacity > 0 and state.media_title and state.media_title ~= '' then
		local title_size = math.max(20 * state.scale, self.font_size * 1.45)
		local title_y = bay - math.max(14 * state.scale, title_size * 1.05)
		ass:txt(
			bax + ((bbx - bax) / 2),
			title_y,
			8,
			ass_escape(state.media_title),
			{
				size = title_size,
				color = fgt,
				border = options.text_border * state.scale,
				border_color = bg,
				opacity = text_opacity * 0.95,
			}
		)
	end
